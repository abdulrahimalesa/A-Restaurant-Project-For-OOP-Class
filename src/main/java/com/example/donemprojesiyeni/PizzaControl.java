...
public class PizzaControl {
    ...
    @FXML
    private TextField PizaaAdi;

    @FXML
    private TextField PizaaAciklama;

    @FXML
    private TextField PizaaFiyat;
    ...
    public void fillData(ObservableList<data> dList){
        ...
        for(data d : dList){
            ...
            sil.setOnAction(event -> {
                Dosyaİslemleri service = new Dosyaİslemleri();
                service.silme("pizaa", finalIndex+1);
                dataList.getChildren().clear();
                fillData(service.Okumak("pizaa"));
            });
    ...
    @FXML
    void PizaaKaydet(ActionEvent event){
        data d = new data(PizaaAdi.textProperty(), PizaaAciklama.textProperty(), PizaaFiyat.textProperty());

        Dosyaİslemleri service = new Dosyaİslemleri();
        service.kaydet(d, "pizaa");

        closeStage(event);
    }

    @FXML
    void pizaaEkle(ActionEvent event) throws IOException {
        FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/com/example/donemprojesiyeni/pizzaekle.fxml"));
